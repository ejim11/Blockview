  ...
  return (
    <>
      ...
      backgroundAttachment: "fixed"
          ...
          <motion.h1 initial = "up" animate = "down"  transition = {{duration: .7}} variants = {textDown} className="text-[5.5rem] sm:text-[3.5rem]">
        ...
        <section className="py-[8rem] px-[10rem] xlg:px-[5rem] sm:px-[2rem]">
         {homeData.map((data,i) =>  <motion.div  initial={{ opacity: 0 }} transition = {{duration: 3}} whileInView={{ opacity: 1 }} key={i} className="flex items-center  justify-between my-[8rem] sm:px-5 mt-12 md:flex-col">
            <div className={`w-[45%] xmd:w-[50%]  md:w-full md:text-center ${i%2 === 0 ? "order-1": "order-2 md:order-1"}`}>
              ...
              <p className="mt-4 text-[1.8rem] text-justify sm:text-center">
               {data.text}
              </p>
            </div>
