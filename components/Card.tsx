...
const Card: React.FC<{ children: React.ReactNode, prev ?: PageNavButtons , next ?: PageNavButtons , title?: string, bgImg?: string  }> = ({ children, prev, next, title, bgImg }) => {
  ...
  return (
    <section className=" w-[calc(100vw-8rem)] sm:w-[calc(100vw-7rem)] relative mt-[8rem] sm:mt-[7rem] h-[calc(100vh-8rem)] sm:h-[calc(100vh-7rem)] overflow-y-scroll  " ref={containerRef} >
        <motion.div
            style={{ scaleX: scrollYProgress }}
            className="fixed h-[1rem] top-[8rem] sm:top-[7rem] left-[8rem] sm:left-[7rem]  right-0 origin-left w-full bg-color-light-blue z-20"
        />
      ...
      <div className=" w-full text-[1.7rem] bg-color-white  p-[3rem] sm:border-0 sm:p-[1.5rem]">
        {children}
      </div>
