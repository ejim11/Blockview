...
const LibraryNavBar = () => {
  ...
  const libraryIsVisible: boolean = useAppSelector(state => state.library.libraryVisible)
  ...
  const chosenTopic: Topic | undefined = topics.find((topic: Topic): boolean| undefined => {
  ...
  return (
    <motion.div
      className={`flex mt-[8rem] sm:absolute sm:left-0 sm:top-[7rem] sm:mt-0 z-40 sm:bg-color-dark-blue ${libraryIsVisible? "sm:opacity-100 sm:visible": "sm:opacity-0 sm:invisible sm:-z-10"}`}
      initial = {false}
      ref={containerRef}
